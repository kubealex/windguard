...
def setup_mqtt_client() -> mqtt.Client:
    ...
    try:
        ...
        client.loop_start()  # Start a background thread to handle MQTT network loop
        return client
...
def get_anomaly_score(endpoint_url: str, row_data: dict) -> float:
    ...
    try:
        ...
        response = requests.post(endpoint_url, json=native_payload, timeout=5)
...
def process_data_file(filepath: str, features: list[tuple[str, str]], client: mqtt.Client, score_url: str):
    ...
    try:
        ...
        for chunk in pd.read_csv(filepath, chunksize=1):
            ...
            score = get_anomaly_score(score_url, row_dict)
...
def main():
    ...
    if not mqtt_client:
        logging.error("Could not instantiate MQTT client. Exiting.")
        return
